...
description: |
  ...
  v5.1.0 added `widgets:` as a REQUIRED field on every view YAML.
  Phase 5 validate-widgets reads this list to know which view to land
  on before lite-invoking each widget. Plugin commit: 406e53aa.
  ...
  Replaces the v3.2.1 bootstrap-era body. The v60 prompt emits a
  widgets-first manifest tree with widgets as the primary unit (every
  selector lives in widgets.yaml, actions are thin compositions).
